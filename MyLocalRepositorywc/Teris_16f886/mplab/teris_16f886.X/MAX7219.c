...
#define DATA_AN_PORT      A
#define DATA_AN_POS       3
#define DATA_AN_PIN       PIN(DATA_PORT, DATA_POS)

#define LOAD_PORT         A
#define LOAD_POS          2
#define LOAD_PIN          PIN(LOAD_PORT, LOAD_POS)
#define LOAD              PORTBIT(LOAD_PIN)
#define LOAD_AN_PORT      A
#define LOAD_AN_POS       2
#define LOAD_AN_PIN       PIN(LOAD_PORT, LOAD_POS)

#define CLK_PORT          A
#define CLK_POS           1
#define CLK_PIN           PIN(CLK_PORT, CLK_POS)
#define CLK               PORTBIT(CLK_PIN)
#define CLK_AN_PORT       A
#define CLK_AN_POS        1
#define CLK_AN_PIN        PIN(CLK_PORT, CLK_POS)
...
uint8_t cur_scroll_position = 0;
uint16_t cur_letter_position = 0;
uint16_t cur_letter_end_position = 0;
uint8_t temp_char_spacing = 0;
char* scroll_chars;
uint8_t cur_char_data[8] = {0, 0, 0, 0, 0, 0, 0, 0};
uint8_t scroll_text_len = 0;
uint8_t const font_width[] = {
...
uint8_t const numbers[] = {0x3F, 0x30, 0x5B, 0x4F, 0x66, 0x6D, 0x7D, 0x07, 0x7F, 0x6F};
uint8_t nd;
uint8_t pos1;
//concat two 8bit int and gives a 16 bit int
//helper function

uint16_t concat(uint8_t d1, uint8_t d2) {
    uint16_t temp;
    temp = 0x0000;
    temp = d1;
    shift_left(&temp, 2, 0);
...
uint8_t reverse(uint8_t nd_data) {
    uint8_t D[8];
    uint8_t i;
    uint8_t temp;
    for (i = 0; i < 8; i++) {
...
void display_digits(uint8_t arr[], uint8_t dp) {
    uint8_t i;
    //int decimal;
    if (dp > 8 || dp < 1)
        dp = 0;
...
void display_char(char ch) {
    int char_offset = 0;
    int8_t char_width = 0;
    int8_t char_data[8] = {0, 0, 0, 0, 0, 0, 0, 0};
    int8_t j = 0;
    uint16_t i = 0;
    uint16_t sum = 0;
...
void put_number(uint8_t* str, uint8_t align, uint8_t pad_zeros) 
{
    uint8_t len = 0;
    int8_t i = 0, j = 0;
    uint8_t char_data[8] = {0, 0, 0, 0, 0, 0, 0, 0};
    len = strlen(str);
    if (align == 0) {
        for (i = 0; i < len; i++) {
            if (str[i] == '.') {
...
void display_init(uint8_t decode) 
{
    IO_OUTPUT(DATA_PIN);
